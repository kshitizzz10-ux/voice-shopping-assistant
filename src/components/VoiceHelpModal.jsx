    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-xl max-h-[90vh] flex flex-col overflow-hidden animate-scale-up">
        ...
        <div className="p-4 sm:p-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Natural voice phrases you can speak anytime</p>
            </div>
          </div>
              ...
              <div className="space-y-1.5">
                {commands.map((cmd) => (
                  <button
                    ...
                    className="w-full text-left p-3 rounded-2xl bg-slate-50 hover:bg-emerald-50/70 border border-slate-200/70 hover:border-emerald-300 transition-all flex items-center justify-between group"
                  >
                    <span className="text-sm font-medium text-slate-800 group-hover:text-emerald-950">
                      "{cmd}"
                    </span>
          ...
          <div className="pt-3 border-t border-slate-200">
            ...
            <div className="space-y-2">
              {multilingualExamples.map((item) => (
                <div
                  key={item.lang}
                  className="p-3 bg-emerald-50/40 rounded-2xl border border-emerald-100 text-xs"
                >
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between text-xs text-slate-500">
          <span>💡 Tap any command to simulate voice input</span>
          <button
            type="button"
            onClick={onClose}
